...
def upload_image(session, image_path):
    ...
    # INIT
    r = session.post(UPLOAD_URL, data={
        "command": "INIT",
        "total_bytes": len(data),
        "media_type": mime,
    }, headers={"Content-Type": "application/x-www-form-urlencoded"})
    rj = r.json()
    print(f"  🔍 Upload INIT response: {rj}")
    media_id = rj["media_id_string"]
    ...
    session.post(UPLOAD_URL, data={
        ...
        "media_id": media_id,
        "segment_index": 0,
    }, files={"media": data}, headers={"Content-Type": None})
...
def post_tweet(session, text, reply_to_id=None, media_id=None):
    ...
    tweet_id = r.json()["data"]["create_tweet"]["tweet_results"]["result"]["rest_id"]
    return tweet_id
...
def post_thread(account, tweets, acct_index):
    ...
    for i, text in enumerate(tweets):
        ...
        if i == 0 and image_path:
            ...
            media_id = upload_image(session, image_path)
